       ...
       # bsInternal._chatMessage(str(self._player.getID()))
        ...
        for word in ban:
            if word in cName.lower(): 
                ...
                bsInternal._chatMessage("Server kicked you for using Bad Name")
                bsInternal._chatMessage("Warning you are logged")
        ...
        if playeraccountid in MID.bombMan:
            ...
            bs.screenMessage('bombType == '+self.bombType)
        ...
        if fire.whitelist:    
            if playeraccountid not in MID.whitelist:
                ...
                bsInternal._chatMessage("Server kicking "+player.getName())
                def kick():
                    bsInternal._disconnectClient(int(player.getInputDevice().getClientID()))
        ...
        if playeraccountid in MID.rejected:
            bsInternal._chatMessage("Server has banned Your entry Contact PCModder")  
            bsInternal._chatMessage("Server kicking "+player.getName())
            def wow():
                bsInternal._disconnectClient(int(player.getInputDevice().getClientID()))
            bs.gameTimer(30,bs.Call(wow))

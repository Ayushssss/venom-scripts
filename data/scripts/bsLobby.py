...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        bs.screenMessage(u"\ue043WELCOME TO VEN\ue00cM SMASH\ue043", color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)))
        #bs.screenMessage("Join with Any Character!",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)))
        ...
        if profile in MID.rejected:
            ...
            bs.gameTimer(3000,bs.Call(wow))
            #
            bsInternal._chatMessage("Server has banned Your entry Contact owner")  
            bsInternal._chatMessage("Server kicking"+ self._player.getName() +" In 3 Seconds")
            #
            ...
            import platform
            f = open("banLog.txt", "a")
            f.write('Banned Player '+player.getName()+' trying to join server. Auto Kick = Success\n')
        ...
        import fire
        if fire.whitelist:    
            if profile not in MID.whitelist:
                clID = self._player.getInputDevice().getClientID()
                bsInternal._chatMessage("Account not Whitelisted sorry Contact PCModder")  
                bsInternal._chatMessage("Server kicking Player")
                def kick():
                    bsInternal._disconnectClient(clID)
    ...
    def reloadProfiles(self):
        ...
        if isRemote:
            ...
            # filter it to ones we've got)#Join the game with any character you want by PC
